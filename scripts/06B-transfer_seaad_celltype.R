...
rds_files<-list.files('~/tcwlab/ShareSpace/MIT_ROSMAP_Multiomics/',pattern = '.rds',full.names = T)
for(f in rds_files){
  ...
  if(!'predicted.id'%in%colnames(obj[[]])){
    ...
    dlpfc.anchors <- FindTransferAnchors(reference = dlpfc, query = obj, dims = 1:50,
